...
if ! grep -q '"version": "0.38.0"' "$ROOT/package.json"; then
  printf 'Patch files were not copied completely. Expected package version 0.38.0.\n' >&2
  exit 1
fi
...
printf 'PROSPECT 0.38.0 FULL-FIELD REAL-TIME GAMEPLAY applied.\nRun: npm run test; npm run build; npm run dev\n'
