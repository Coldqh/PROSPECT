...
if ! grep -q '"version": "0.37.0"' "$ROOT/package.json"; then
  printf 'Patch files were not copied completely. Expected package version 0.37.0.\n' >&2
  exit 1
fi

rm -f "$ROOT/src/components/career/MatchField.tsx"
...
printf 'PROSPECT 0.37.0 REAL-TIME FOOTBALL ENGINE applied.\nRun: npm run test; npm run build; npm run dev\n'
