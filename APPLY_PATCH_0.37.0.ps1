...
if ($Version -ne "0.37.0") {
  throw "Patch files were not copied completely. Expected package version 0.37.0, found $Version."
}

$OldField = Join-Path $ProjectRoot "src\components\career\MatchField.tsx"
if (Test-Path $OldField) {
  Remove-Item -Force $OldField
  Write-Host "Deleted legacy scripted MatchField.tsx"
}
...
Write-Host "PROSPECT 0.37.0 REAL-TIME FOOTBALL ENGINE applied."
