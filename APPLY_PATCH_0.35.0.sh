...
if ! grep -q '"version": "0.35.0"' "$ROOT/package.json"; then
  printf 'Patch files were not copied completely. Expected package version 0.35.0.\n' >&2
  exit 1
fi
...
printf 'PROSPECT 0.35.0 ALL PLAYABLE POSITIONS applied.\nRun: npm run test; npm run build; npm run dev\n'
