...
foreach ($RelativePath in $Required) {
  ...
  if (-not (Test-Path $Path)) {
    throw "Required 0.42.0 file is missing: $RelativePath. Extract the patch into C:\PROSPECT with replacement."
  }
}

$Package = Get-Content (Join-Path $ProjectRoot "package.json") -Raw
if ($Package -notmatch '"version"\s*:\s*"0\.42\.0"') {
  throw "package.json is not PROSPECT 0.42.0. Check the extraction directory."
}

$DeleteList = Join-Path $ProjectRoot "DELETE_FILES_0.42.0.txt"
...
Write-Host "PROSPECT 0.42.0 installed."
