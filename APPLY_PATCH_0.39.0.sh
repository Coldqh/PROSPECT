...
if ! grep -q '"version": "0.39.0"' "$ROOT/package.json"; then
  printf 'Patch files were not copied completely. Expected package version 0.39.0.\n' >&2
  exit 1
fi
...
printf 'PROSPECT 0.39.0 AUTONOMOUS PROFESSIONAL LEAGUE applied.\nRun: npm run test; npm run build; npm run dev\n'
